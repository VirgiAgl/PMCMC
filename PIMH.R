...
PIMH = function(n_iter, 
  ...
  n_acceptance=0
  state_values = matrix(NA, nrow = N, ncol = n_iter+1 ) #for the state values at each step
  lik_values = vector(length = n_iter+1 )               #for the lik values at step
...
PIMH_toy = PIMH(n_iter, 
                N, 
                observed_process_toy, 
                calculate_weight=calculate_weight_toy,  
                state_update=toy_state_update, 
                observed_process=observed_process_toy
                )

plot1=tracePlot(PIMH_toy$state_values[1,], 
                n_iter, 
