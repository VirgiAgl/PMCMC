...
generate_data = function (state_update, obs_update, prior_par, t, plot=TRUE){
  ...
  observed_process[1] = obs_update(x_k = latent_process[1])
  
  # propagate the state space model
  for (i in 2:t){
    latent_process[i] = state_update(x_k = latent_process[i-1], k = i - 1)
    observed_process[i] = obs_update(x_k = latent_process[i])
  }
